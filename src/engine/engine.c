...
struct engine {
     ...
     * Объекты, которым нужен лишь определенный набор подсистем, вынужденны включать заголовки, где содержатся упоминания огромного количества нерелевантных подсистем (пусть хоть и через forward declaration).
     ...
     * Если выносить запрос к подсистемам на awake (кеширование на awake стадартная практика) то при неверном ключе программа будет падать хоть и в рантайме, но на самом ее старте.
     */
